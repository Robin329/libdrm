...
extern int set_frame_buff(int width, int height, unsigned char *cemaro_buf);
...
int main()
{
	...
	int fd = open(DEV_PATH, O_RDWR,
		      0); //弄了好久 以阻塞模式打开摄像头  | O_NONBLOCK 非堵塞
	if (fd < 0) {
	...
	req.memory =
		V4L2_MEMORY_MMAP; //automation or user define．自动分配还是自定义
