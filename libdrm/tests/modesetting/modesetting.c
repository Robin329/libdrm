...
bool create_fb(int drm_fd, uint32_t width, uint32_t height,
	       struct dumb_framebuffer *fb)
{
	int ret;
	...
	ret = drmModeAddFB2(drm_fd, width, height, DRM_FORMAT_XRGB8888, handles,
			    strides, offsets, &fb->id, 0);
	if (ret < 0) {
	...
	fb->data = mmap(0, fb->size, PROT_READ | PROT_WRITE, MAP_SHARED, drm_fd,
			map.offset);
	if (!fb->data) {
		perror("mmap");
		goto error_fb;
...
int main(void)
{
	...
	for (int i = 0; i < res->count_connectors; ++i) {
		...
		// Perform the modeset
		int ret = drmModeSetCrtc(drm_fd, conn->crtc_id, conn->fb.id, 0,
					 0, &conn->id, 1, &conn->mode);
		if (ret < 0) {
	...
	for (int i = 0; i < 60 * 5; ++i) {
		...
		for (struct connector *conn = conn_list; conn;
		     conn = conn->next) {
			if (!conn->connected)
				continue;
