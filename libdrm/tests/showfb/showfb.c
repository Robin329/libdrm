...
int main(int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < planes->count_planes; i++) {
		...
		uint64_t plane_type = get_property_value(
			drm_fd, plane_id, DRM_MODE_OBJECT_PLANE, "type");
	...
	uint32_t fb_id = 0;
	drmModeAddFB2(drm_fd, width, height, DRM_FORMAT_XRGB8888, handles,
	...
	uint8_t *data = mmap(0, size, PROT_READ | PROT_WRITE, MAP_SHARED,
			     drm_fd, map.offset);
	...
	add_property(drm_fd, req, plane_id, DRM_MODE_OBJECT_PLANE, "FB_ID",
		     fb_id);
	...
	add_property(drm_fd, req, plane_id, DRM_MODE_OBJECT_PLANE, "SRC_W",
		     width << 16);
	add_property(drm_fd, req, plane_id, DRM_MODE_OBJECT_PLANE, "SRC_H",
		     height << 16);
	...
	add_property(drm_fd, req, plane_id, DRM_MODE_OBJECT_PLANE, "CRTC_W",
		     width);
	add_property(drm_fd, req, plane_id, DRM_MODE_OBJECT_PLANE, "CRTC_H",
		     height);
