...
static int set_drm_object_property(drmModeAtomicReq *req,
				   struct drm_object *obj, const char *name,
				   uint64_t value)
{
	int i;
	uint32_t prop_id = 0;
			     ...
			     struct modeset_output *out)
{
	...
	for (i = 0; i < conn->count_encoders; ++i) {
		...
		for (j = 0; j < res->count_crtcs; ++j) {
			...
			if (crtc > 0) {
				fprintf(stdout,
					"crtc %u found for encoder %u, will need full modeset\n",
...
static int modeset_find_plane(int fd, struct modeset_output *out)
{
	...
	for (i = 0; (i < plane_res->count_planes) && !found_primary; i++) {
		...
		if (plane->possible_crtcs & (1 << out->crtc_index)) {
			 ...
			 */
			if (get_property_value(fd, props, "type") ==
						    ...
						    drmModeConnector *conn)
{
	...
	fprintf(stderr, "mode for connector %u is %ux%u\n", conn->connector_id,
		out->bufs[0].width, out->bufs[0].height);
					 ...
					 drmModeAtomicReq *req)
{
	...
	if (set_drm_object_property(req, &out->connector, "CRTC_ID",
				    out->crtc.id) < 0)
		return -1;
	...
	if (set_drm_object_property(req, &out->crtc, "MODE_ID",
				    out->mode_blob_id) < 0)
		return -1;
