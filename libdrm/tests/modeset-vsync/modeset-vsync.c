...
int main(int argc, char **argv)
{
	...
	for (iter = modeset_list; iter; iter = iter->next) {
		...
		ret = drmModeSetCrtc(fd, iter->crtc, buf->fb, 0, 0, &iter->conn,
				     1, &iter->mode);
		if (ret)
			fprintf(stderr,
...
static void modeset_draw_dev(int fd, struct modeset_dev *dev)
{
	...
	ret = drmModePageFlip(fd, dev->crtc, buf->fb, DRM_MODE_PAGE_FLIP_EVENT,
			      dev);
	if (ret) {
	...
	while (modeset_list) {
		...
		if (!iter->pflip_pending)
			drmModeSetCrtc(fd, iter->saved_crtc->crtc_id,
				       ...
				       iter->saved_crtc->x, iter->saved_crtc->y,
				       &iter->conn, 1, &iter->saved_crtc->mode);
