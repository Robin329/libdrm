...
int main(int argc, char **argv)
{
	...
	drmModeSetCrtc(fd, crtc_id, buf.fb_id, 0, 0, &conn_id, 1,
		       &conn->modes[0]);
	printf("input 'a' 'y' 'r' 'g' 'b' to set the screen color\n");
	while (1) {
		r = getchar();
		switch (r) {
		case 'y':
			write_color(&buf, 0x00ffff00);
			break;
		case 'a':
			write_color(&buf, 0xff000000);
			break;
		case 'r':
			write_color(&buf, 0x00ff0000);
			break;
		case 'g':
			write_color(&buf, 0x0000ff00);
			break;
		case 'b':
			write_color(&buf, 0x000000ff);
			break;
		default:
			break;
		};
