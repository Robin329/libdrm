...
static int modeset_create_fb(int fd, struct buffer_object *bo)
{
	...
	bo->vaddr = mmap(0, create.size, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
			 map.offset);
	...
	ret = drmModeAddFB2(fd, bo->height, bo->height, DRM_FORMAT_XRGB8888,
...
static int modeset_create_yuvfb(int fd, struct buffer_object *bo)
{
	...
	ret = drmModeAddFB2(fd, bo->width, bo->height, DRM_FORMAT_YUV422,
			    handles, pitches, offsets, &bo->fb_id, 0);
	if (ret) {
		printf("drmModeAddFB2 return err %d\n", ret);
		return 0;
...
void set_rotation(int fd, int plane_id, int value)
{
	...
	prop.value =
		value; //rotate-0=0x1 rotate-90=0x2 rotate-180=0x4 rotate-270=0x8, modetest -p can show the details
...
int main(int argc, char **argv)
{
	...
	drmModeSetCrtc(fd, crtc_id, buf.fb_id, 0, 0, &conn_id, 1,
		       &conn->modes[0]);
	...
	ret = drmModeSetPlane(fd, plane_res->planes[3], crtc_id,
			      plane_buf[2].fb_id, 0, x, y, plane_buf[2].width,
			      plane_buf[2].height, 0 << 16, 0 << 16,
